...
export function CollectionSidebar({
  ...
  return (
    <aside className="h-full lg:max-h-[calc(100vh-110px)] lg:overflow-auto rounded-lg">
      <div className="w-full md:w-64 lg:w-72 2xl:w-80 flex flex-col gap-6 bg-[#122D42] py-6 px-4 rounded-lg ">
        {/* Collection Image */}
        <div className="rounded-lg   bg-slate-800 aspect-square">
          <Image
            width={302}
            height={281}
            src={collection?.image || "/placeholder.svg"}
            alt={collection?.title}
            className="w-full h-[281px] object-cover rounded-lg"
          />
        </div>
